            ...
            if (data.status) {
                for (let i = 0; i < saveMembers.length; i++) {
                    if (saveMembers[i].status) {
                        ...
                        const mailing = sgMail.send(emailConstant.servicerApproval(checkPrimaryEmail2.email, { link: resetLink, subject: "Reset Password", role: "Customer", servicerName: saveMembers[i].firstName }))
                    }
        ...
        if (data.status) {
            for (let i = 0; i < saveMembers.length; i++) {
                if (saveMembers[i].status) {
                    ...
                    let checkPrimaryEmail2 = await userService.updateSingleUser({ email: email }, { resetPasswordCode: resetPasswordCode }, { new: true });
                    let resetLink = `${process.env.SITE_URL}newPassword/${checkPrimaryEmail2._id}/${resetPasswordCode}`
                    const mailing = sgMail.send(emailConstant.servicerApproval(checkPrimaryEmail2.email, { link: resetLink, subject: "Reset Password", role: "Reseller", servicerName: saveMembers[i].firstName }))
                }
            }
        }
        //Save Logs for create reseller 
