...
module.exports = class dealerService {
  ...
  static async createRelationWithServicer(data) {
    try {
      const relation = await relationTable.create(data);
      return relation;
    } catch (error) {
      return `Could not create the relation: ${error}`;
    }
  }
  ...
  static async getDealerRelation(query, projection) {
    try {
      const relation = await relationTable.findOne(query, projection);
      return relation;
    } catch (error) {
  ...
  static async editDealerRelation(criteria, data, option) {
    try {
      const updatedRelation = await relationTable.findOneAndUpdate(criteria, data, option);
