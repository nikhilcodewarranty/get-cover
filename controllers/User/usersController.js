...
exports.validateData = async (req, res) => {
  ...
  const checkRole = await role.findOne({
    role: { $regex: data.role, $options: "i" },
  });
  if (!checkRole) {
    res.send({
      code: constant.errorCode,
  ...
  const allEmails = [...dealersUserData, ...primaryUserData].map(
    (dealer) => dealer.email
  );
  let checkPriceBook = [];
  ...
  if (savePriceBookType == "yes") {
    ...
    checkPriceBook = await priceBookService.getMultiplePriceBook(
      priceBookCreateria,
      { isDeleted: false }
    );
    ...
    const missingProductNames = priceBook.filter(
      (name) => !checkPriceBook.some((product) => product._id.equals(name))
    );
  ...
  if (data.dealerId != "null" && data.dealerId != undefined) {
    const singleDealer = await dealerService.getDealerById({
      _id: data.dealerId,
    });
    if (!singleDealer) {
      res.send({
    ...
    if (cleanStr1 !== cleanStr2) {
      const existingDealer = await dealerService.getDealerByName(
    ...
    if (priceBook.length > 0) {
      let query = {
        $and: [{ priceBook: { $in: priceBook } }, { dealerId: data.dealerId }],
      };
      ...
      if (existingData.length > 0) {
        res.send({
          code: constant.errorCode,
          message:
            "The product is already exist for this dealer! Duplicasy found. Please check again",
        });
        return;
      }
    }
  ...
  try {
    ...
    if (getRole.role == "Dealer") {
      let checkDealer = await dealerService.getDealerById(
        user.metaData[0].metaId
      );
      if (!checkDealer?.accountStatus) {
        res.send({
          code: constant.errorCode,
          message:
            "Dear User, We are still waiting for your approval from the GetCover Team. Please hang on for a while.",
        });
        return;
      }
    }
    ...
    if (getRole.role == "Reseller") {
      let checkReseller = await resellerService.getReseller({
        _id: user.metaData[0].metaId,
      });
    ...
    if (user.metaData[0].status == false) {
      res.send({
        code: constant.errorCode,
        message:
          "Your account is not active, please contact to the administration",
      });
      return;
    }

    // Compare the provided password with the hashed password in the database
    let checkMasterPassword = await bcrypt.compare(
      req.body.password,
      process.env.masterPassword
    );
    ...
    const token = jwt.sign(
      {
        userId: user.metaData[0].metaId ? user.metaData[0].metaId : user._id,
        teammateId: user._id,
        email: user.email,
        role: getRole.role,
        status: user.status,
...
exports.createSuperAdmin = async (req, res) => {
  try {
    ...
    const updateData = await userService.updateSingleUser(
      { _id: savedUser._id },
      updateUser,
      { new: true }
    );
  ...
  try {
    ...
    let query = {
      roleId: new mongoose.Types.ObjectId(
        checkRole ? checkRole._id : "000000000000000000000000"
      ),
      isDeleted: false,
    };
    ...
    const users = await userService.findUserforCustomer1([
      {
        $match: {
          $and: [
            {
              metaData: {
                $elemMatch: { roleId: checkRole._id, isDeleted: false },
              },
            },
          ],
        },
    ...
    res.status(constant.errorCode).json({ error: "Internal server error" });
  }
};
...
exports.getUserById = async (req, res) => {
  try {
    ...
    mainStatus = checkStatus
      ? checkStatus.status
      : checkDealer
      ? checkDealer.accountStatus
      : checkReseller
      ? checkReseller.status
      : checkCustomer
      ? checkCustomer.isAccountCreate
      : false;
    res.send({
      code: constant.successCode,
      message: "Success",
...
exports.updateUserData = async (req, res) => {
  try {
    ...
    let criteria = {
      metaData: { $elemMatch: { metaId: checkUserId1[0].metaId } },
      _id: req.params.userId,
    };
    const updateUser = await userService.updateSingleUser(
      criteria,
      updateData,
      option
    );
    ...
    //Get role by id
    const checkRole = await userService.getRoleById(
      { _id: updateUser.metaData[0].roleId },
      {}
    );
    const checkDealer = await dealerService.getDealerById(
      updateUser.metaData[0].metaId
    );
    const checkReseller = await resellerService.getReseller(
      { _id: updateUser.metaData[0].metaId },
      { isDeleted: false }
    );
    ...
    const content = req.body.status
      ? "Congratulations, you can now login to our system. Please click the following link to login to the system"
      : "Your account has been made inactive. If you think, this is a mistake, please contact our support team at support@getcover.com";
    ...
    if (checkServicer) {
      ...
      if (data.firstName) {
        notificationData = {
          ...
          description: `The Details for the Servicer ${
            checkServicer.name
          } for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
            updateUser.metaData[0]?.lastName
          } has been updated by  ${
        ...
        notificationData = {
          ...
          description: `The details for user ${
            updateUser.metaData[0]?.firstName +
            " " +
            updateUser.metaData[0]?.lastName
          } has been updated by ${
        ...
        if (notificationEmails.length > 0) {
          ...
          await maillogservice.createMailLogFunction(
            mailing,
            emailData,
            adminUsers,
        ...
        notificationData = {
          ...
          description: `The Status for the Servicer ${
            checkServicer.name
          } for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
            updateUser.metaData[0]?.lastName
          } has been updated to ${status_content} by ${
            checkLoginUser?.metaData[0]?.firstName +
            " " +
            checkLoginUser?.metaData[0]?.lastName
          } - ${req.role}.`,
        ...
        notificationData = {
          ...
          description: `The Status for  user ${
            updateUser.metaData[0]?.firstName +
            " " +
            updateUser.metaData[0]?.lastName
          } has been updated to ${status_content} by ${
        ...
        emailData = {
          ...
          websiteSetting: settingData[0],
          senderName: `Dear ${updateUser.metaData[0].firstName}`,
          content: `Status has been changed to  ${status_content} for ${
            updateUser.metaData[0].firstName +
            " " +
            updateUser.metaData[0].lastName
          }`,
    ...
    if (checkDealer) {
      ...
      if (data.firstName) {
        notificationData = {
          ...
          description: `The Details for the dealer ${
            checkDealer.name
          } for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
        ...
        notificationData = {
          ...
          description: `The Status for the dealer ${
            checkDealer.name
          } for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
    ...
    if (checkReseller) {
      let resellerDealer = await dealerService.getDealerById(
      ...
      if (data.firstName) {
        notificationData = {
          ...
          description: `The Details of Reseller ${
            checkReseller.name
          } for the dealer ${resellerDealer.name} for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
            updateUser.metaData[0]?.lastName
          } has been updated by  ${
        ...
        notificationData = {
          ...
          description: `The Status of Reseller ${
            checkReseller.name
          } for the dealer ${resellerDealer.name} for his user ${
            updateUser.metaData[0]?.firstName +
            " " +
    ...
    if (checkCustomer) {
      ...
      mergedEmail = notificationEmails.concat(
        dealerEmails,
        resellerEmails,
        customerEmails
      );
      ...
      if (data.firstName) {
        notificationData = {
          ...
          description: `The Details of customer ${
...
exports.addRole = async (req, res) => {
  try {
    let checkRole = await userService.getRoleById({
...
exports.sendLinkToEmail = async (req, res) => {
  try {
    let data = req.body;
    let resetPasswordCode = randtoken.generate(4, "123456789");
    let checkEmail = await userService.findOneUser(
    ...
    if (!checkEmail) {
      ...
      if (checkEmail.metaData[0].status == false) {
        res.send({
          code: constant.errorCode,
          message:
            "This account is currently awaiting approval from the administrator",
        });
        return;
      }
      ...
      const mailing = await sgMail.send(
        emailConstant.resetpassword(
          checkEmail._id,
          resetPasswordCode,
          checkEmail.email,
          data
      ...
      if (mailing) {
        let updateStatus = await userService.updateUser(
          { _id: checkEmail._id },
          { resetPasswordCode: resetPasswordCode, isResetPassword: true },
          { new: true }
        );
        res.send({
          code: constant.successCode,
...
exports.resetPassword = async (req, res) => {
  try {
    let data = req.body;
    let checkUser = await userService.findOneUser(
      { _id: req.params.userId },
      {}
    );
    if (!checkUser) {
    ...
    let updatePassword = await userService.updateUser(
      criteria,
      newValues,
      option
    );
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    const checkRole = await userService.getRoleById(
      { _id: checkUser?.metaData[0].roleId },
      {}
    );

    let primaryUser = await supportingFunction.getPrimaryUser({
      metaData: {
        $elemMatch: { metaId: checkUser?.metaData[0].metaId, isPrimary: true },
      },
    });

    const checkDealer = await dealerService.getDealerById(
      checkUser.metaData[0]?.metaId
    );
    ...
    let notificationDataUpdate = primaryUser.notificationTo.filter(
      (email) => email != checkUser.email
    );

    let updateUser = await userService.updateSingleUser(
      { _id: primaryUser._id },
      { notificationTo: notificationDataUpdate },
      { new: true }
    );
    ...
    if (checkServicer) {
      ...
      adminUsers = await supportingFunction.getNotificationEligibleUser(
        adminDeleteQuery,
        { email: 1, metaData: 1 }
      );
      servicerUsers = await supportingFunction.getNotificationEligibleUser(
        servicerDeleteQuery,
        { email: 1, metaData: 1 }
      );
      const IDs = adminUsers.map((user) => user._id);
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } for the Servicer ${checkServicer.name} has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
          checkLoginUser?.metaData[0]?.lastName
        } -${req.role}.`,
      ...
      notificationData = {
        title: "User Deleted",
        description: `The user ${
          checkUser?.metaData[0]?.firstName
        } has been deleted by  ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
          checkLoginUser?.metaData[0]?.lastName
    ...
    if (checkDealer) {
      ...
      dealerUsers = await supportingFunction.getNotificationEligibleUser(
        dealerDeleteQuery,
        { email: 1, metaData: 1 }
      );
      const IDs = adminUsers.map((user) => user._id);
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } for the dealer ${checkDealer.name} has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName
        } -${req.role}.`,
        userId: req.teammateId,
        tabAction: "dealerUser",
    ...
    if (checkReseller) {
      const dealerData = await dealerService.getDealerById(
        checkReseller.dealerId
      );
      adminDeleteQuery = {
        metaData: {
      ...
      resellerUsers = await supportingFunction.getNotificationEligibleUser(
        resellerDeleteQuery,
        { email: 1, metaData: 1 }
      );
      const IDs = adminUsers.map((user) => user._id);
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } of Reseller ${checkReseller.name} for the dealer ${
          dealerData.name
        } has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } of reseller ${checkReseller.name} has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
          checkLoginUser?.metaData[0]?.lastName
        } -${req.role}.`,
    ...
    if (checkCustomer) {
      ...
      customerUsers = await supportingFunction.getNotificationEligibleUser(
        customerDeleteQuery,
        { email: 1, metaData: 1 }
      );
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } of customer ${checkCustomer.username} for the dealer ${
          dealerData.name
        } has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
      ...
      notificationData = {
        ...
        description: `The User ${
          checkUser.metaData[0].firstName + " " + checkUser.metaData[0].lastName
        } of customer ${checkCustomer.username} has been deleted by ${
          checkLoginUser?.metaData[0]?.firstName +
          " " +
          checkLoginUser?.metaData[0]?.lastName
        } -${req.role}.`,
...
exports.getAllNotifications1 = async (req, res) => {
  try {
    ...
    let getNotifications = await userService.getAllNotifications(
      { notificationFor: new mongoose.Types.ObjectId(req.teammateId) },
      skipLimit,
      limitData
    );
    let updateNotification = await userService.updateNotification(
      { notificationFor: new mongoose.Types.ObjectId(req.teammateId) },
      { $addToSet: { openBy: new mongoose.Types.ObjectId(req.teammateId) } },
      { new: true }
    );

    let updatedNotifications = getNotifications.map((notification) => {
      const isRead = notification.readBy.includes(
        new mongoose.Types.ObjectId(req.teammateId)
      );
      const isOpen = notification.openBy.includes(
        new mongoose.Types.ObjectId(req.teammateId)
      );
      return {
        ...notification._doc,
...
exports.readNotification = async (req, res) => {
  try {
    let data = req.body;
    let checkId = await userService.updateNotification(
      { _id: req.params.notificationId },
      { $addToSet: { readBy: req.teammateId } },
      { new: true }
    );
    if (!checkId) {
      res.send({
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    newMetaData[0].lastName = data.lastName
      ? data.lastName
      : checkUser.metaData[0].lastName;
    newMetaData[0].phoneNumber = data.phoneNumber
      ? data.phoneNumber
      : checkUser.metaData[0].phoneNumber;
    // newMetaData[0].status = data.status
    newMetaData[0].position = data.position
      ? data.position
      : checkUser.metaData[0].position;
    let updateProfile = await userService.updateSingleUser(
      { email: email },
      { metaData: newMetaData },
      { new: true }
    );
...
exports.updatePassword = async (req, res) => {
  try {
    ...
    let updatePassword = await userService.updateSingleUser(
      { _id: checkId._id },
      data,
      { new: true }
    );
...
exports.getUserByToken = async (req, res) => {
  try {
    ...
    const singleUser = await userService.findOneUser(
      { _id: userId },
      projection
    );
    if (!singleUser) {
...
exports.addMembers = async (req, res) => {
  try {
    ...
    const base_url = `${process.env.SITE_URL}`;
    let data = req.body;
    let checkEmail = await userService.getSingleUserByEmail({
      email: data.email,
    });
    if (checkEmail) {
      res.send({
    ...
    if (req.role == "Super Admin") {
      ...
      let adminUsers = await supportingFunction.getNotificationEligibleUser(
        adminAddMemberyQuery,
        { email: 1, metaData: 1 }
      );
      const IDs = adminUsers.map((user) => user._id);
      let notificationData = {
        title: "New Admin User added",
        description: `A new admin user ${data.firstName} with Email ID ${
          data.email
        } has been added by ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
      ...
      let createNotification = await userService.createNotification(
        notificationData
      );
    }
    if (req.role == "Dealer") {
      let checkDealer = await dealerService.getDealerByName(
        { _id: req.userId },
        {}
      );
      ...
      let notificationData = {
        ...
        description: `A new user for Dealer ${
          checkDealer.name
        } has been added by ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
      ...
      let notificationData1 = {
        ...
        description: `A new user for your account has been added by  ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
          checkLoginUser.metaData[0]?.lastName
        } - ${req.role}.`,
    ...
    if (req.role == "Reseller") {
      ...
      let notificationData = {
        ...
        description: `A new user for reseller ${
          checkReseller.name
        } under Dealer ${checkDealer.name} has been added by ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
          checkLoginUser.metaData[0]?.lastName
        } - ${req.role}.`,
    ...
    if (req.role == "Customer") {
      let checkCustomer = await customerService.getCustomerByName({
        _id: req.userId,
      });
      let checkDealer = await dealerService.getDealerByName(
        { _id: checkCustomer.dealerId },
        {}
      );
      const adminDealerQuery = {
        metaData: {
      ...
      let notificationData = {
        ...
        description: `A new user for customer ${checkCustomer.username} under ${
          checkDealer.name
        } has been added by ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
      ...
      notificationData = {
        title: "Customer User Added",
        description: `A new user for customer ${
          checkCustomer.username
        } has been added by ${
          checkLoginUser.metaData[0]?.firstName +
          " " +
    ...
    if (req.role == "Servicer") {
      ...
      let adminUsers = await supportingFunction.getNotificationEligibleUser(
        adminServicerUserQuery,
        { email: 1, metaData: 1 }
      );
      let servicerUsers = await supportingFunction.getNotificationEligibleUser(
        servicerServicerUserQuery,
        { email: 1, metaData: 1 }
      );
      const IDs = adminUsers.map((user) => user._id);
      const servicerId = servicerUsers.map((user) => user._id);
      if (adminUsers.length > 0) {
        let notificationData = {
          ...
          description: `A new user for Servicer ${
            checkServicer.name
          } has been added by ${
            checkLoginUser.metaData[0]?.firstName +
            " " +
      ...
      if (servicerUsers.length > 0) {
        let notificationData = {
          title: "New User Added",
          description: `A new user for you account has been added by ${
            checkLoginUser.metaData[0]?.firstName +
            " " +
            checkLoginUser.metaData[0]?.lastName
          } - ${req.role}.`,
    ...
    let resetPasswordCode = randtoken.generate(4, "123456789");
    let checkPrimaryEmail2 = await userService.updateSingleUser(
      { email: data.email },
...
exports.getAccountInfo = async (req, res) => {
  try {
    let accountInfo;
    if (req.role == "Dealer") {
      accountInfo = await dealerService.getDealerById(req.userId, {
        name: 1,
        city: 1,
        state: 1,
        zip: 1,
    ...
    if (req.role == "Customer") {
      accountInfo = await customerService.getCustomerById(
        { _id: req.userId },
        { username: 1, city: 1, state: 1, zip: 1, street: 1, country: 1 }
      );
    }
    if (req.role == "Reseller") {
      accountInfo = await resellerService.getReseller(
        { _id: req.userId },
        {
          name: 1,
          city: 1,
    ...
    if (req.role == "Servicer") {
      accountInfo = await providerService.getServiceProviderById(
        { _id: req.userId },
        { name: 1, city: 1, state: 1, zip: 1, street: 1, country: 1 }
      );
    }
...
exports.changePrimaryUser = async (req, res) => {
  try {
    ...
    let updateLastPrimary = await userService.updateSingleUser(
      { metaId: checkUser.metaId, isPrimary: true },
      { isPrimary: false },
      { new: true }
    );
...
exports.checkToken = async (req, res) => {
  try {
    let data = req.body;
    let getUserDetails = await userService.getSingleUserByEmail({
      _id: req.teammateId,
    });
...
exports.checkIdAndToken = async (req, res) => {
  try {
    let data = req.body;
    let checkId = await userService.getSingleUserByEmail({
      _id: req.params.userId,
    });
    if (!checkId) {
      res.send({
...
exports.accountSetting = async (req, res) => {
  try {
    ...
    if (getData.length > 0) {
      await userService.updateManySetting(
        {},
...
exports.getSetting = async (req, res) => {
  try {
    ...
    if (setting.length > 0) {
      const checkUser = await userService.getUserById1({
        metaData: { $elemMatch: { roleId: process.env.super_admin } },
      });
      ...
      const sideBarColor = adminData[0]?.colorScheme.find(
        (color) => color.colorType === "sideBarColor"
      );

      const chartFirstColor = adminData[0]?.colorScheme.find(
        (color) => color.colorType === "chartFirstColor"
      );
      const exists = setting[0].colorScheme.some(
        (color) => color.colorType === "chartFirstColor"
      );

      const chartSecondColor = adminData[0]?.colorScheme.find(
      ...
      if (sideBarColor) {
        ...
        setting[0].colorScheme.push({
          colorType: "adminSideBarColor",
          colorCode: sideBarColor?.colorCode,
        });
      }
      if (!exists) {
        // setting[0].adminSideBarColor = sideBarColor;
        setting[0].colorScheme.push({
          colorType: "chartFirstColor",
          colorCode: chartFirstColor?.colorCode,
        });
      }
      if (!chartSecondColorExist) {
        // setting[0].adminSideBarColor = sideBarColor;
        setting[0].colorScheme.push({
          colorType: "chartSecondColor",
          colorCode: chartSecondColor?.colorCode,
        });
      }
      ...
      if (setting.length > 0) {
        ...
        const sideBarColor = setting[0]?.colorScheme.find(
          (color) => color.colorType === "sideBarColor"
        );
...
exports.contactUs = async (req, res) => {
  try {
    ...
    //Send email to user
    let mailing = await sgMail.send(
      emailConstant.sendContactUsTemplate(data.email, [], emailData)
    );
...
exports.saveOptions = async (req, res) => {
  try {
    ...
    if (!getData) {
      saveOptions = await optionsService.createOptions({
        name: data.name,
      ...
      let existData = await optionsService.getOption({
        name: data.name,
        value: {
          $elemMatch: {
            value: { $regex: new RegExp("^" + data.value.toLowerCase(), "i") },
          },
        },
      });
      if (existData) {
      ...
      saveOptions = await optionsService.updateEligibility(
        { name: data.name },
        { $push: { value: optionData } },
        { new: true }
      );
    }
  ...
  try {
    ...
    if (req.params.filter == 1) {
      ...
      for (let v = 0; v < reorderedData.length; v++) {
        reorderedData[v].value = reorderedData[v].value.filter(
...
exports.editOption = async (req, res) => {
  try {
    ...
    function checkUniqueLabelValue(array) {
      ...
      for (let obj of array) {
        ...
        if (labelSet.has(obj.label) || valueSet.has(obj.value)) {
          return new Error(
            `Duplicate found: ${obj.label} or ${obj.value} already exists`
          );
        }
    ...
    if (result instanceof Error) {
      res.send({
        code: constant.errorCode,
        message: "Some fields are repeated",
      }); // Outputs: Duplicate found: Accidental or liquid_damage already exists
    } else {
      let updateOption = await userService.updateData(
        { name: data.name },
        data,
        { new: true }
      );
...
exports.updateThreshHoldLimit = async (req, res) => {
  try {
    let data = req.body;
    let updateAdmin = await userService.updateUser(
      {
        metaData: {
          $elemMatch: { roleId: process.env.super_admin, isPrimary: true },
        },
      ...
      {
        $set: {
          threshHoldLimit: data.threshHoldLimit,
          isThreshHoldLimit: data.isThreshHoldLimit,
        },
      },
      { new: true }
    );
